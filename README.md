...
district_df["Total Schools"] = district_df["Total Schools"].map("{:,.0f}".format)
...
format_main_df = main_df.copy()
format_main_df['Total School Budget'] = format_main_df['Total School Budget'].map('${:,.2f}'.format)
format_main_df['Per Student Budget'] = format_main_df['Per Student Budget'].map('${:,.2f}'.format)
format_main_df.sort_index(inplace=True)
format_main_df
```
...
```python
format_main_df.sort_values(by=('% Overall Passing Rate'), ascending=False).head(5)
```
...
```python
format_main_df.sort_values(by=('% Overall Passing Rate'), ascending=True).head(5)
```
